...
2. **同一組操作按鈕內只放一個實心主色鈕**
   範圍指「同一個按鈕群組」——最典型是 **table 上方的操作功能鈕列**，其次是卡片 / 彈窗的 footer。同一群組內盡量不要同時出現兩顆以上實心主色按鈕，否則彼此稀釋、使用者不知該按哪個。不同區塊（如 table 工具列 vs 另一區的搜尋列 vs 彈窗 footer）各自可有一個主色 CTA，不受此限（不是整個畫面只能有一顆）。
3. **顏色不可單獨承載語意**
...
- **標題 + 副標**放搜尋 Card 內左上，不另立頁首標題（呼應 7.6）。
- **頁首操作鈕列**（批次作業、匯出、設定入口…）放搜尋 Card 右上；**全部用 outlined**（次要視覺），主色 CTA 留給表單/彈窗（見 6.9）。下拉型操作用 PrimeVue `<Menu :popup>`（`#start`/`#end` slot 放說明與提示），不要手刻 `<button>`。
...
- 矩陣式表單（label × 維度）用 grid：維度標頭用 `<Tag :severity>`（不手刻上色）；缺格用「—」佔位，**高度對齊輸入框 44px**；不分維度的列讓其輸入框 `col-span-*` 橫跨維度欄，併入同一矩陣而非另開區塊。
- 拿不準時自問：使用者是來「**填**」還是「**看**」？填 → grid / form；看 → DataTable。

### 6.9 頁面操作按鈕分群

後台頁面的操作鈕依「所在群組」判定主色鈕規則（見 5.2）——每一組內只放一個實心主色鈕：

| 群組 | 位置 | 慣例 |
| --- | --- | --- |
| **頁首操作列** | 列表頁搜尋 Card 右上 | **多個並列操作**時全部 outlined（次要），避免與表單/彈窗的主色 CTA 打架（如訂單頁：批次作業 / 合併訂單 / 匯出）；若頁首**只有單一主導的「新增 / 建立」動作**，該動作可用實心主色 CTA（其餘輔助操作仍 outlined）——如多購物車頁的「新增多購物車」、商品頁的建立按鈕 |
| **搜尋 / 篩選列** | 搜尋 Card 內 | 「搜尋」為該組主色鈕；其餘（清除、進階篩選切換）次要 |
| **Table 操作欄** | 每列最右 | icon-only + tooltip + `aria-label`；編輯=主色、刪除=danger、其餘 secondary（見 7.5） |
| **彈窗 footer** | Dialog / Drawer 底部 | 主要動作（儲存 / 確認 / 送出）為該組唯一實心主色鈕，靠右；取消 outlined secondary（見 7.4） |

不同群組各自可有一顆主色 CTA，彼此不衝突。
