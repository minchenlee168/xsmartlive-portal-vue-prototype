...
const FILLER_COMBOS: Array<{ ss: OrderRow['shippingStatus']; ps: OrderRow['paymentStatus'] }> = [
  ...
  { ss: 'cancelled',  ps: 'pending_refund' },// 已取消 · 待退款
  { ss: 'cancelled',  ps: 'pending_refund' },// 已取消 · 待退款(多給一些)
  { ss: 'completed',  ps: 'paid' },          // 已完成(補量)
  { ss: 'cancelled',  ps: 'refunded' },      // 已取消 · 已退款
  { ss: 'cancelled',  ps: 'refunded' },      // 已取消 · 已退款
]
...
function openCancelOrderDialog(): void {
  ...
  cancelBlockedKind.value = ['completed', 'cancelled'].includes(o.shippingStatus)
    ? 'completed'
    : 'shipped'
...
const BATCH_CONFIGS: Record<Exclude<BatchMode, null>, BatchConfig> = {
  ...
  shipping_list: {
    ...
    // 尚未結束的訂單(排除已完成 / 已取消 等終止狀態)都可列印出貨單
    isSelectable: (o) => !['completed', 'cancelled'].includes(o.shippingStatus),
    performAction: () => {
      // 列印動作交給實際列印邏輯,mock 只跳 toast
    },
      ...
      <div v-if="detailDialogOrder" class="flex flex-col gap-3">
        ...
        <div class="rounded-md border border-[var(--p-primary-200)] bg-[var(--p-primary-50)] px-3 py-2.5 flex flex-col gap-1">
          ...
          <span class="text-sm text-[var(--p-primary-color)]">
            {{ cancelBlockedKind === 'completed'
              ? '已結案的訂單不再開放取消或編輯,如有爭議請另行與買家聯繫處理。'
